...
define('FRK_MOBILE_ATTACHMENT_FOLDER', 'mobile-files/');//mod for android file up-downloads

//echo FRK_MOBILE_ATTACHMENT_FOLDER; die;
		...
		if ($objFileList->rMore()) {
			while ($objFile = $objFileList->rNext()) {
				...
				$str .= '<tr id="vfile'.$objFile->id.'">'
					...
					.'<td><div class="vnewaction" style="font-size:11px"><a onClick="this.blur()" href="mobile-files/'.$objFile->id.'-'.$objFile->filename.'" download="'.$objFile->filename.'">'
...
function task_file_delete($fileId) {
	...
	if ($objFile = ajaxLoadFile($fileId,4,$objUser,$objTask,$objResponse)) {
		...
		$newtempfilename=$objFile->id."-".$objFile->filename;// delete the mobile file too 
		unlink(FRK_MOBILE_ATTACHMENT_FOLDER.$newtempfilename);
		//echo $objFile->filename;die;//
		$objFile->deleteFile();
...
function task_file_upload($taskId,$file,$desc,$tempfile) {
	...
	if ($objTask = ajaxLoadTask($taskId,2,true,$objUser,$objResponse)) {
		...
		if ($objFile->uploadFile()) {
			...
			// Copy original file, rename file by id.frk and remove temp
			//copy(FRK_ATTACHMENT_FOLDER.$tempfile, FRK_MOBILE_ATTACHMENT_FOLDER.$objFile->id."-".$tempfile); //for android and mobile downloading that don't like the Java
			copy(FRK_ATTACHMENT_FOLDER.$tempfile, FRK_ATTACHMENT_FOLDER.$objFile->id.".frk");
			$newtempfilename=$objFile->id."-".$objFile->filename;
			copy(FRK_ATTACHMENT_FOLDER.$objFile->id.".frk", FRK_MOBILE_ATTACHMENT_FOLDER.$newtempfilename);
			
			//copy(FRK_MOBILE_ATTACHMENT_FOLDER.$objFile->id.".frk", FRK_MOBILE_ATTACHMENT_FOLDER.$objFile->id."-".$objFile->filename);
